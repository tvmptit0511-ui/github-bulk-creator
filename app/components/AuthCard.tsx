  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button className="btn-green" onClick={() => verifyToken()} disabled={!token.trim() || status === 'checking'} style={{ minWidth: 80 }}>
          {status === 'checking' ? <Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> : 'Xác thực'}
        </button>
      </div>

      {status === 'error' && (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 8, color: 'var(--danger)', fontSize: 12 }}>
      ...
      <p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 8 }}>
        ...
        <a href="https://github.com/settings/tokens/new?scopes=repo&description=GH+Bulk+Creator" target="_blank" rel="noreferrer" style={{ color: 'var(--blue)' }}>
          Tạo token tại đây ↗
        </a>
      </p>
